											...
											<form action="{{url('admin/activated/'.$data->id)}}" method="post">
												...
												<div class="modal-body">
													<select name="activated" class="form-control" id="">
														<option value="0" {{$data->email_verified == 0 ? 'selected' : ''}}>Tidak Aktif</option>
														<option value="1" {{$data->email_verified == 1 ? 'selected' : ''}}>Aktif</option>
													</select>
													<br>
													<input type="checkbox" name="centang" onclick="var input = document.getElementById('pass{{$data->id}}'); if( this.checked){ input.disabled = false; input.focus();}else{input.disabled=true;}"> Ganti Password<br>
