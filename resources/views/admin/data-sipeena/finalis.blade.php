...
@endif
<!--- End Alert Modal -->
<h4>Data Finalis Pendaftaran SiPeena</h4>
<div class="row">
...
<script type="text/javascript">
	...
	$(function() {
		var table = $('#individu').DataTable({
			...
			processing: true,
			serverSide: true,
			ajax: "{{ route('admin.getFinalisIndividu') }}",
			columns: [{
					name: 'DT_RowIndex',
					data: 'DT_RowIndex',
					orderable: false,
	...
	$(function() {
		var table = $('#kelompok').DataTable({
			...
			processing: true,
			serverSide: true,
			ajax: "{{ route('admin.getFinalisKelompok') }}",
			columns: [{
					name: 'DT_RowIndex',
					data: 'DT_RowIndex',
					orderable: false,
	...
	$(function() {
		var table = $('#lembaga').DataTable({
			...
			processing: true,
			serverSide: true,
			ajax: "{{ route('admin.getFinalisLembaga') }}",
			columns: [{
					name: 'DT_RowIndex',
					data: 'DT_RowIndex',
					orderable: false,
	...
	$(function() {
		var table = $('#opd').DataTable({
			...
			processing: true,
			serverSide: true,
			ajax: "{{ route('admin.getFinalisOpd') }}",
			columns: [{
					name: 'DT_RowIndex',
					data: 'DT_RowIndex',
					orderable: false,
